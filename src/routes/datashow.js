...
// ?date=2026-05-21&time=09:00&duration=60
router.get('/check', auth, (req, res) => {
  const { date, time, duration, exclude_id } = req.query;
  if (!date || !time) return res.status(400).json({ error: 'date e time obrigatorios' });
  const dur = parseInt(duration) || 60; // minutes
  const [sh, sm] = time.split(':').map(Number);
  const startMin = sh * 60 + sm;
  const endMin   = startMin + dur;

  db.datashow.find({ date }, (err, reservations) => {
    const conflicts = (reservations || []).filter(r => {
      if (exclude_id && r._id === exclude_id) return false;
      const [rh, rm] = r.time.split(':').map(Number);
      const rStart = rh * 60 + rm;
      const rEnd   = rStart + (parseInt(r.duration) || 60);
      return startMin < rEnd && endMin > rStart; // overlap
    });
...
router.post('/', auth, (req, res) => {
  ...
  // Conflict check
  db.datashow.find({ date }, (err, existing) => {
